# app/modules/module_9/pdf_routes.py
...
@module_9_pdf.route("/download_pdf/<int:application_id>", methods=["GET"])
@login_required
def download_pdf(application_id):
    ...
    if current_user.id not in allowed_users or application.status not in [
        ...
        logger.warning(f"Unauthorized PDF download by user {current_user.id}, app {application_id}")
    ...
    # Styles setup (aligned with Module 7 and 8)
    ...
    # Track attachment positions for Module 9
    attachment_positions = {}

    # PDF Document setup for Module 9
    ...
    summary_flowables = []
    summary_flowables.append(Paragraph("Module 9: Application Summary", title_style))
    ...
    for md in processed_module_9_data:
        ...
        toc_index += 1
    toc_items.append(["", "Uploaded Documents (Module 9)", ""])
    ...
    # Module 9 Section
    section_index = 1
    for i, md in enumerate(processed_module_9_data):
        section_flowables = []
        ...
        if i < len(processed_module_9_data) - 1:
    ...
    # Uploaded Documents Section (Module 9 only)
    uploaded_flowables = []
    uploaded_flowables.append(Paragraph("Uploaded Documents (Module 9)", heading_style))
    doc_list = []
    ...
    # Build the initial Module 9 PDF with ReportLab
    doc.build(story, onFirstPage=header_footer, onLaterPages=header_footer)

    # Use PyMuPDF to add attachments and links
    final_doc = fitz.open(summary_pdf_path)

    # Track page count for Module 9 content
    module_9_page_count = len(final_doc)

    # Add Module 9 attachment pages
    attachment_page_numbers = {}
    current_page = module_9_page_count
    ...
    for filename, file_path in attachment_files:
        ...
        separator_page.insert_text(
            (page_width / 2 - fitz.get_text_length("Attachment (Module 9)", fontname="helv", fontsize=14) / 2, 100),
            "Attachment (Module 9)",
            fontname="helv",
            fontsize=14,
            color=(0, 0, 0),
    ...
    # Add back links for Module 9 attachments
