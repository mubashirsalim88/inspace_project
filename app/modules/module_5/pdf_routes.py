# app/modules/module_5/pdf_routes.py
...
module_5_pdf = Blueprint(
    "module_5_pdf", __name__, url_prefix="/module_5", template_folder="templates"
)
...
@module_5_pdf.route("/download_pdf/<application_id>", methods=["GET"])
@login_required
def download_pdf(application_id):
    ...
    # Fetch Module 5 data
    module_5_data = ModuleData.query.filter_by(
        application_id=application_id, module_name="module_5"
    ).all()
    processed_module_5_data = [
        {"step": md.step, "data": md.data.copy(), "completed": md.completed}
        for md in module_5_data
        if md.step.lower() != "summary"  # Exclude 'summary' step
    ]

    # Fetch uploaded files for Module 5 from UploadedFile
    uploaded_files_module_5 = UploadedFile.query.filter_by(
        application_id=application_id, module_name="module_5"
    ).all()
    uploaded_file_paths = {}
    for f in uploaded_files_module_5:
    ...
    for md in processed_module_5_data:
    ...
    # Create temporary files for Module 5 PDF
    ...
    # Track attachment positions for Module 5
    attachment_positions = {}

    # PDF Document setup for Module 5
    ...
    summary_flowables = []
    summary_flowables.append(Paragraph("Module 5: Host Space Object Details", title_style))
    ...
    for md in processed_module_5_data:
        ...
        toc_index += 1
    toc_items.append(["", "Uploaded Documents (Module 5)", ""])
    ...
    # Module 5 Section
    section_index = 1
    for i, md in enumerate(processed_module_5_data):
        section_flowables = []
        ...
        step_files = [f for f in uploaded_files_module_5 if f.step == md["step"]]
        for f in step_files:
            absolute_path = os.path.join(os.path.dirname(__file__), "..", f.filepath)
            if absolute_path not in files_added and os.path.exists(absolute_path):
        ...
        if i < len(processed_module_5_data) - 1:
    ...
    # Uploaded Documents Section (Module 5 only)
    uploaded_flowables = []
    uploaded_flowables.append(Paragraph("Uploaded Documents (Module 5)", heading_style))
    doc_list = []
    ...
    # Build the initial Module 5 PDF with ReportLab
    doc.build(story, onFirstPage=header_footer, onLaterPages=header_footer)

    # Use PyMuPDF to add attachments and links
    final_doc = fitz.open(summary_pdf_path)

    # Track page count for Module 5 content
    module_5_page_count = len(final_doc)

    # Add Module 5 attachment pages
    attachment_page_numbers = {}
    current_page = module_5_page_count  # Start after Module 5 content
    ...
    for _, file_path in attachment_files:
        ...
        separator_page.insert_text(
            (page_width / 2 - fitz.get_text_length("Attachment (Module 5)", fontname="helv", fontsize=14) / 2, 100),
            "Attachment (Module 5)",
            fontname="helv",
            fontsize=14,
            color=(0, 0, 0),
    ...
    for file_path in attachment_positions:
        if file_path in attachment_page_numbers:
            ...
            page_num = 0
            while page_num < module_5_page_count:  # Only in Module 5 content pages
    ...
    # Add back links for Module 5 attachments
    ...
    try:
        response = send_file(
            final_pdf_path,
            as_attachment=True,
            download_name=f"application_{application_id}_module_5_summary.pdf",
...
@module_5_pdf.route("/test_pdf")
def test_pdf():
    return "module_5_pdf Blueprint is working!"