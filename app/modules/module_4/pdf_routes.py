# app/modules/module_4/pdf_routes.py
...
def download_pdf(application_id):
    ...
    # Define expected file fields per step
    file_fields = {
    ...
    # Fetch uploaded files for Module 4 from UploadedFile
            ...
            for field in file_fields[step]:
                ...
                if isinstance(value, list) and all(isinstance(v, str) for v in value):
                    for file_path in value:
                        ...
                        logger.debug(f"ModuleData: Step: {step}, Field: {field}, File: {os.path.basename(file_path)}, Path: {absolute_path}, Exists: {os.path.exists(absolute_path)}")
                else:
                    logger.debug(f"ModuleData: Step: {step}, Field: {field}, Value: {value} (not a list of paths)")
    ...
    # Create temporary files for Module 4 PDF
    ...
    # Track attachment positions for Module 4
    attachment_positions = {}

    # PDF Document setup for Module 4
    ...
    summary_flowables = []
    summary_flowables.append(Paragraph("Module 4: Additional Satellite Form Summary", title_style))
                ...
                if isinstance(value, list) and all(isinstance(v, str) for v in value):
                    for file_path in value:
                        ...
                        if absolute_path not in files_added and os.path.exists(absolute_path):
                            ...
                            logger.debug(f"Added {filename} from ModuleData.data[{field}] for step {step}")
        # Include non-file fields
        for key, value in md["data"].items():
            if key not in file_fields.get(step, []) and key not in ["documents", "document_names"] and value:
                data_table.append([
    ...
    # Uploaded Documents Section (Module 4 only)
    uploaded_flowables = []
    ...
    # Build the initial Module 4 PDF with ReportLab
    doc.build(story, onFirstPage=header_footer, onLaterPages=header_footer)

    # Use PyMuPDF to add attachments and links
    final_doc = fitz.open(summary_pdf_path)

    # Track page count for Module 4 content
    module_4_page_count = len(final_doc)

    # Add Module 4 attachment pages
    attachment_page_numbers = {}
    current_page = module_4_page_count  # Start after Module 4 content
    ...
    for _, file_path in attachment_files:
        ...
        separator_page.insert_text(
            (page_width / 2 - fitz.get_text_length("Attachment (Module 4)", fontname="helv", fontsize=14) / 2, 100),
            "Attachment (Module 4)",
            fontname="helv",
            fontsize=14,
            color=(0, 0, 0),
    ...
    for file_path in attachment_positions:
        if file_path in attachment_page_numbers:
            ...
            page_num = 0
            while page_num < module_4_page_count:  # Only in Module 4 content pages
    ...
    # Add back links for Module 4 attachments
