# app/modules/module_11/routes.py
...
module_11 = Blueprint("module_11", __name__, template_folder="templates")
...
@module_11.route("/fill_step/<step>", methods=["GET", "POST"])
@login_required
def fill_step(step):
    ...
    module_data = ModuleData.query.filter_by(application_id=app_id, module_name="module_11", step=step).first()
    if not module_data:
        module_data = ModuleData(application_id=app_id, module_name="module_11", step=step, data={})
        db.session.add(module_data)
        db.session.commit()

    existing_files = UploadedFile.query.filter_by(application_id=app_id, module_name="module_11", step=step).all()
    ...
    if request.method == "POST" and step != "miscellaneous_and_declarations":
        ...
        if next_step_idx < len(STEPS):
            return redirect(url_for("module_11.fill_step", step=STEPS[next_step_idx], application_id=app_id))
        return redirect(url_for("module_11.fill_step", step="summary", application_id=app_id))

    if step == "summary":
        all_module_data = ModuleData.query.filter_by(application_id=app_id, module_name="module_11").all()
...
@module_11.route("/save_miscellaneous_and_declarations/<int:application_id>", methods=["POST"])
@login_required
def save_miscellaneous_and_declarations(application_id):
    ...
    module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_11", step="miscellaneous_and_declarations").first()
    if not module_data:
        module_data = ModuleData(application_id=application_id, module_name="module_11", step="miscellaneous_and_declarations", data={})
        db.session.add(module_data)
...
@module_11.route("/submit_application/<int:application_id>", methods=["POST"])
@login_required
def submit_application(application_id):
    ...
    all_module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_11").all()
    ...
    if not set(required_steps).issubset(completed_steps):
        ...
        return redirect(url_for("module_11.fill_step", step="miscellaneous_and_declarations", application_id=application_id))

    application.status = "Submitted"
    application.editable = False
    db.session.commit()
    flash("Application submitted successfully!", "success")
    return redirect(url_for("module_11.fill_step", step="summary", application_id=application_id))

@module_11.route("/download_file/<int:file_id>")
@login_required
...
@module_11.route("/download_pdf/<application_id>", methods=["GET"])
@login_required
def download_pdf(application_id):
    ...
    all_module_data = ModuleData.query.filter_by(
        application_id=application_id, module_name="module_11"
    ).all()
    processed_module_data = []
    for md in all_module_data:
        data_copy = md.data.copy()
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
        ...
        return send_file(
            pdf_path,
            as_attachment=True,
            download_name=f"application_{application_id}_module_11_summary.pdf",
