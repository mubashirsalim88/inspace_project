# app/modules/module_2/pdf_routes.py
...
def download_pdf(application_id):
    ...
    # Fetch Module 1 data (Basic Detail)
    ...
    # Dictionary to track filenames and their file paths
    unique_attachments = {}
    for filename, filepath in attachment_files:
        if filename not in unique_attachments:
            unique_attachments[filename] = filepath
    attachment_files = [(filename, filepath) for filename, filepath in unique_attachments.items()]
    ...
    # Styles setup (aligned with Module 1)
