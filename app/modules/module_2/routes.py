...
module_2 = Blueprint(
    "module_2", __name__, url_prefix="/module_2", template_folder="templates"
)
...
def fill_step(step):
    ...
    if application.user_id != current_user.id or (
        ...
        logger.warning(
            f"Unauthorized access or invalid application status for user {current_user.id}, app {app_id}"
        )
    ...
    if not module_1_complete:
        flash(
            "Please complete Module 1 (Basic Details) for at least one application before starting Module 2.",
            "error",
        )
        logger.info(
            f"User {current_user.id} redirected to Module 1 due to incomplete Module 1 data"
        )
        existing_module_2_data = ModuleData.query.filter_by(
            application_id=app_id, module_name="module_2"
        ).first()
        ...
        return redirect(
            url_for(
                "module_1.fill_step", step="applicant_identity", application_id=app_id
            )
        )

    module_data = ModuleData.query.filter_by(
        application_id=app_id, module_name="module_2", step=step
    ).first()
    if not module_data:
        module_data = ModuleData(
            application_id=app_id, module_name="module_2", step=step, data={}
        )
    ...
    if request.method == "POST" and step != "misc_and_declarations":
        ...
        file_fields = {
            ...
            "ground_segment": [],
            "itu_and_regulatory": [
                "interference_analysis_indian",
                "interference_analysis_non_indian",
                "annexure_2",
                "launch_service_agreement",
                "insurance_details",
                "dot_license_copies",
            ],
        ...
        return redirect(
            url_for("module_2.fill_step", step="summary", application_id=app_id)
        )

    if step == "summary":
        # Fetch Module 2 data
        all_module_2_data = ModuleData.query.filter_by(
            application_id=app_id, module_name="module_2"
        ).all()
        processed_module_2_data = []
        for md in all_module_2_data:
            data_copy = md.data.copy()
            for key, value in data_copy.items():
                if isinstance(value, list) and all(isinstance(v, str) for v in value):
                    data_copy[key] = [os.path.basename(doc) for doc in value]
            processed_module_2_data.append(
        ...
        return render_template(
            ...
            all_module_2_data=processed_module_2_data,  # Fixed variable name
            application_id=app_id,
    ...
    template_map = {"misc_and_declarations": "module_2/misc_and_declarations.html"}
    return render_template(
...
def save_misc_and_declarations(application_id):
    ...
    # Log field changes
    compare_form_data(
        old_data, form_data, application_id, "module_2", "misc_and_declarations"
    )
    logger.debug(
...
def submit_application(application_id):
    ...
    try:
        ...
        db.session.commit()
        return redirect(
            url_for("module_2.fill_step", step="summary", application_id=application_id)
        )
    except Exception as e:
        db.session.rollback()
...
def start_application():
    if request.method == "POST":
        ...
        db.session.commit()
        return redirect(
            url_for("module_2.fill_step", step=STEPS[0], application_id=application.id)
        )
