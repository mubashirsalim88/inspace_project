...
module_10 = Blueprint("module_10", __name__, template_folder="templates")
...
def fill_step(step):
    ...
    module_data = ModuleData.query.filter_by(application_id=app_id, module_name="module_10", step=step).first()
    if not module_data:
        module_data = ModuleData(application_id=app_id, module_name="module_10", step=step, data={})
        db.session.add(module_data)
        db.session.commit()

    existing_files = UploadedFile.query.filter_by(application_id=app_id, module_name="module_10", step=step).all()
    ...
    if request.method == "POST" and step != "miscellaneous_and_declarations":
        ...
        if next_step_idx < len(STEPS):
            return redirect(url_for("module_10.fill_step", step=STEPS[next_step_idx], application_id=app_id))
        return redirect(url_for("module_10.fill_step", step="summary", application_id=app_id))

    if step == "summary":
        all_module_data = ModuleData.query.filter_by(application_id=app_id, module_name="module_10").all()
...
@module_10.route("/save_miscellaneous_and_declarations/<int:application_id>", methods=["POST"])
@login_required
def save_miscellaneous_and_declarations(application_id):
    ...
    module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_10", step="miscellaneous_and_declarations").first()
    if not module_data:
        module_data = ModuleData(application_id=application_id, module_name="module_10", step="miscellaneous_and_declarations", data={})
        db.session.add(module_data)
...
def submit_application(application_id):
    ...
    all_module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_10").all()
    ...
    if not set(required_steps).issubset(completed_steps):
        ...
        return redirect(url_for("module_10.fill_step", step="miscellaneous_and_declarations", application_id=application_id))
...
@module_10.route("/download_file/<int:file_id>")
@login_required
