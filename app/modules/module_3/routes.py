...
def download_pdf(application_id):
    ...
    module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_3").all()
    uploaded_files = UploadedFile.query.filter_by(application_id=application_id, module_name="module_3").all()
    ...
    = Application {{ application_id }} - Module 3 Summary
    ...
    response = send_file(final_pdf, as_attachment=True, download_name=f"Module_3_Application_{application_id}.pdf")
    os.remove(final_pdf)
    return response
