...
def download_pdf(application_id):
    ...
    # Fetch uploaded files for Module 1 and Module 7
    ...
    uploaded_file_paths = {}
    for f in uploaded_files_module_1 + uploaded_files_module_7:
    ...
    # Fetch file paths from ModuleData.data for Module 1 and Module 7
    module_data_file_paths = {}
    for md in processed_module_1_data + processed_module_7_data:
    ...
    for md in processed_module_1_data:
        ...
        toc_index += 1
    toc_items.append([f"{toc_index}.", "Module 7: Launch Authorization Details", ""])
    toc_index += 1
    ...
    # Module 7 Section
    story.append(Paragraph(f"{section_index}. Module 7: Launch Authorization Details", heading_style))
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.gray, spaceBefore=10, spaceAfter=10))
    section_index += 1
    for md in processed_module_7_data:
        section_flowables = []
                ...
                if isinstance(value, list) and all(isinstance(v, str) for v in value):
                    for file_path in value:
                        ...
                        if absolute_path not in files_added and os.path.exists(absolute_path):
                            ...
                            files_added.add(absolute_path)
                            logger.debug(f"Added {filename} from ModuleData.data[{key}] for Module 7 step {md['step']}")
                else:
                    data_table.append([
                        Paragraph(f"{key_clean}:", label_style),
                        Paragraph(str(value), normal_style),
        ...
        for f in step_files:
            ...
            if absolute_path not in files_added and os.path.exists(absolute_path):
                ...
                files_added.add(absolute_path)
                logger.debug(f"Added {f.filename} from UploadedFile for Module 7 step {md['step']}, field {f.field_name}")
        if data_table:
            table = Table(data_table, colWidths=[150, 360])
            table.setStyle([
                ("FONTSIZE", (0, 0), (-1, -1), 11),
