        ...
        if step in file_fields:
            for field in file_fields[step]:
                ...
                if files and files[0].filename:
                    file_paths = []
                    for f in files:
                        filename = f"{app_id}_{step}_{field}_{uuid.uuid4()}_{f.filename}"
...
def save_undertaking_declaration(application_id):
    ...
    for field in file_fields:
        ...
        if files and files[0].filename:
            file_paths = []
            for f in files:
                filename = f"{application_id}_undertaking_declaration_{field}_{uuid.uuid4()}_{f.filename}"
                file_path = os.path.join(UPLOAD_FOLDER, filename)
                f.save(file_path)
                relative_path = os.path.join("uploads", filename)
                uploaded_file = UploadedFile(
...
@module_7.route("/download_pdf/<int:application_id>")
@login_required
def download_pdf(application_id):
    ...
    module_data = ModuleData.query.filter_by(application_id=application_id, module_name="module_7").all()
    uploaded_files = UploadedFile.query.filter_by(application_id=application_id, module_name="module_7").all()
    ...
    = Application {{ application_id }} - Module 7 Summary
    ...
    response = send_file(final_pdf, as_attachment=True, download_name=f"Module_7_Application_{application_id}.pdf")
    os.remove(final_pdf)
    return response